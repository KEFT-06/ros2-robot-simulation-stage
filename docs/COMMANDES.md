...
## 3. Gazebo — le bateau qui flotte (coque horizontale)
```bash
ros2 launch mon_robot_bringup simulation.launch.py
```
Options (pose de la coque) :
```bash
ros2 launch mon_robot_bringup simulation.launch.py boat_z:=0.05 boat_yaw:=0.0
...
# options :
ros2 launch mon_robot_bringup demo.launch.py run_trajectory:=true trajectory_type:=complex record_bag:=true
```
...
| Terminal | Commande |
|---|---|
| **1** | sourcing → `ros2 launch mon_robot_bringup simulation.launch.py` (Gazebo, coque flotte à plat) |
| **2** | sourcing → `ros2 run mon_robot_control send_trajectory.py complex` (la tête/hélice bouge) |
| **3** | sourcing → `ros2 run mon_robot_control validate_fk.py` (vérifie la position effecteur) |
